...
def main():
    ...
    with col1:
        # Collect user data
        input_data['Pclass'] = st.selectbox('Passenger Class', [1,2,3])
        input_data['Sex'] = st.selectbox('Sex', ['male', 'Female'])
        input_data['Age'] = st.number_input('Passenger Age', step=1)
        
    with col2:
        input_data['SibSp'] = st.selectbox("Passenger's siblings onboard", [0,1,2,3,4,5])
        input_data['Parch'] = st.selectbox("Passenger's Parents and Children Onboard", [0,1,2,3,4,5,6])
